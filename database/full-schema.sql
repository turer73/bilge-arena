...
-- ============================================================
-- 086: Kazanim / Hakimiyet Haritasi Pilotu
-- Ilk veri dilimi TYT Matematik / sayilar icin kategori-seviyesi pilot kanit.
-- Resmi MEB kazanim taksonomisi degildir.
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_curriculum_outcomes_game_exam_active
  ON curriculum_outcomes (game, exam_ref, sort_order) WHERE is_active = TRUE;
CREATE INDEX IF NOT EXISTS idx_question_outcomes_outcome
  ON question_outcomes (outcome_id, question_id);
...
ALTER TABLE user_outcome_state ENABLE ROW LEVEL SECURITY;

CREATE OR REPLACE FUNCTION record_outcome_evidence()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  INSERT INTO user_outcome_state (
  ...
  SELECT NEW.user_id, mapping.outcome_id, 1,
    ...
    NEW.answered_at,
    NOW()
  FROM question_outcomes AS mapping
  JOIN curriculum_outcomes AS outcome ON outcome.id = mapping.outcome_id
  WHERE mapping.question_id = NEW.question_id AND outcome.is_active = TRUE
  ON CONFLICT (user_id, outcome_id) DO UPDATE SET
    attempts = user_outcome_state.attempts + EXCLUDED.attempts,
    correct_attempts = user_outcome_state.correct_attempts + EXCLUDED.correct_attempts,
    weighted_earned = user_outcome_state.weighted_earned + EXCLUDED.weighted_earned,
    weighted_possible = user_outcome_state.weighted_possible + EXCLUDED.weighted_possible,
    delayed_correct = user_outcome_state.delayed_correct + EXCLUDED.delayed_correct,
    last_answered_at = GREATEST(user_outcome_state.last_answered_at, EXCLUDED.last_answered_at),
    updated_at = NOW();
  RETURN NEW;
END;
$$;
...
CREATE TRIGGER trg_record_outcome_evidence
  ...
  FOR EACH ROW EXECUTE FUNCTION record_outcome_evidence();

REVOKE ALL ON curriculum_outcomes FROM anon, authenticated;
REVOKE ALL ON question_outcomes FROM anon, authenticated;
REVOKE ALL ON user_outcome_state FROM anon, authenticated;
GRANT ALL ON curriculum_outcomes TO service_role;
GRANT ALL ON question_outcomes TO service_role;
GRANT ALL ON user_outcome_state TO service_role;

INSERT INTO curriculum_outcomes (
  code, game, category, title, description, exam_ref, sort_order, is_active
)
VALUES (
  'MAT-SAY-01', 'matematik', 'sayilar', 'Sayılar ve işlem becerisi (pilot)',
...
SELECT
  ...
  1,
  NOT EXISTS (
    SELECT 1 FROM question_outcomes AS existing_primary
...
JOIN curriculum_outcomes AS outcome ON outcome.code = 'MAT-SAY-01'
