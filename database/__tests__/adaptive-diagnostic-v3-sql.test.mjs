...
describe('193 registry-driven adaptive screening contract', () => {
  it('normalizes only the original bounded MAT-SAY-01 provenance before first release', () => {
    const normalizationStart = sql.indexOf('-- Migration 086 created MAT-SAY-01')
    const normalizationEnd = sql.indexOf('CREATE OR REPLACE FUNCTION public.tg_adaptive_diagnostic_blueprint_immutable')
    const blueprintSeed = sql.indexOf('INSERT INTO public.adaptive_diagnostic_blueprints')
    const normalization = sql.slice(normalizationStart, normalizationEnd)

    expect(normalizationStart).toBeGreaterThanOrEqual(0)
    expect(normalizationEnd).toBeGreaterThan(normalizationStart)
    expect(normalizationEnd).toBeLessThan(blueprintSeed)
    expect(normalization).toContain("blueprint_version='ba-tyt-math-diagnostic-v1'")
    expect(normalization).toContain("outcome.code='MAT-SAY-01'")
    expect(normalization).toContain("mapping.created_at=v_outcome_created_at")
    expect(normalization).toContain("mapping.mapping_source='manual'")
    expect(normalization).toContain("SET mapping_source='taxonomy_auto'")
    expect(normalization).toContain("question.game='matematik'")
    expect(normalization).toContain("upper(btrim(COALESCE(question.exam_ref,'')))='TYT'")
    expect(normalization).toContain("question.category='sayilar'")
    expect(normalization).toContain('mapping.is_primary')
    expect(normalization).toContain('other_mapping.outcome_id<>mapping.outcome_id')
    expect(normalization).toContain('v_unsafe_manual_count<>0')
    expect(normalization).toContain('v_legacy_manual_count>0 AND v_legacy_auto_count>0')
    expect(normalization).toContain('v_updated_count<>v_legacy_manual_count')
    expect(normalization).toContain('IF v_blueprint_exists THEN')
    expect(normalization).toContain('IF v_legacy_manual_count<>0 THEN')

    const updateStart = normalization.indexOf('UPDATE public.question_outcomes AS mapping')
    const updateEnd = normalization.indexOf('GET DIAGNOSTICS v_updated_count=ROW_COUNT', updateStart)
    const boundedUpdate = normalization.slice(updateStart, updateEnd)
    expect(updateStart).toBeGreaterThanOrEqual(0)
    expect(updateEnd).toBeGreaterThan(updateStart)
    expect(boundedUpdate).toContain("mapping.outcome_id=v_outcome_id")
    expect(boundedUpdate).toContain("mapping.mapping_source='manual'")
    expect(boundedUpdate).toContain('mapping.created_at=v_outcome_created_at')
    expect(boundedUpdate).toContain('mapping.is_primary')
    expect(boundedUpdate).toContain("question.game='matematik'")
    expect(boundedUpdate).toContain("upper(btrim(COALESCE(question.exam_ref,'')))='TYT'")
    expect(boundedUpdate).toContain("question.category='sayilar'")
    expect(boundedUpdate).toContain('other_mapping.outcome_id<>mapping.outcome_id')
  })
