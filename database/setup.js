/**
 * Bilge Arena — Supabase veritabani kurulum scripti
 * Kullanim: node database/setup.js
 *
 * Schema + migration'lari tek seferde calistirir.
 * .env.local'daki SUPABASE_URL ve db sifresi kullanilir.
 */
...
async function run() {
  console.log('🔌 Supabase PostgreSQL baglantisi kuruluyor...')

  try {
    await client.connect()
    console.log('✅ Baglanti basarili!\n')

    console.log('📦 Schema olusturuluyor...')
    await client.query(SCHEMA_SQL)
    console.log('✅ Schema basariyla olusturuldu!')
    ...
    console.log(`\n📊 Olusturulan tablolar (${rows.length}):`)
    ...
    if (err.message.includes('password')) {
      console.error('   → Veritabani sifresi yanlis olabilir')
    }
    process.exit(1)
  } finally {
    await client.end()
