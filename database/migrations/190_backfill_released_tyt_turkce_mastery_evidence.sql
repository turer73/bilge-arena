-- Migration 190: Backfill historical verified TYT Turkish evidence.
--
-- Migration 189 creates category-proxy mappings after historical attempts may
-- already have received immutable materialization markers. Repair every missing
-- current v2 mapping additively; never infer order from transaction-start NOW().
...
-- A v2 scope owned by migration 189 must be released before its repair. Later
-- taxonomies and operator-retired scopes are intentionally preserved/no-op.
DO $fn$
BEGIN
  IF EXISTS (
    SELECT 1
    ...
    WHERE scope.release_status NOT IN ('released','retired')
  ) THEN
    RAISE EXCEPTION 'TYT Turkish evidence repair requires a released v2 scope'
      USING ERRCODE = '55000';
  END IF;
END
$fn$;
...
SELECT scope.game::text AS game,
  ...
  '190_tyt_turkce_complete_mappings_v2'::text AS repair_key
FROM public.curriculum_scope_releases AS scope
JOIN (VALUES
  ('turkce','TYT','TYT','ba-tyt-turkce-v2')
) AS expected(game,display_exam_ref,question_exam_ref,taxonomy_version)
...
BEGIN
  ...
  IF v_marker_gap <> 0 OR v_snapshot_gap <> 0 THEN
    RAISE EXCEPTION 'TYT Turkish historical mastery provenance is incomplete: marker gaps %, snapshot gaps %',
...
BEGIN
  ...
  IF v_run_count <> v_target_count THEN
    RAISE EXCEPTION 'TYT Turkish evidence repair ledger count mismatch'
      USING ERRCODE = '55000';
  END IF;
  ...
  IF v_target_count = 0 AND (
    ...
    RAISE EXCEPTION 'obsolete TYT Turkish v2 repair mutated rows'
      USING ERRCODE = '23514';
  END IF;
END
$fn$;
