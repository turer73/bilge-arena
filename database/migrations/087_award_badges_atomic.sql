-- Migration 087: Atomic badge awards and XP (disc#1492)
--
-- award_badges is the single writer for user_achievements and its associated
-- badge XP. Its result must remain idempotent: duplicate or concurrent calls
-- return only rows they inserted and award XP only for those rows.
...
BEGIN
  -- Duplicates and unknown codes are ignored. The join back to
  -- inserted_badges makes awards and XP depend only on successful inserts.
  ...
  -- increment_xp writes the XP ledger in the same transaction. Nothing is
  -- written for duplicate, empty, or unknown-only input.
