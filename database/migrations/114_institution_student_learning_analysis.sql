-- Migration 114: tenant-scoped, post-membership student learning analysis.
--
-- This is a read-only projection over verified mastery evidence. It stores no
-- new learning data and deliberately excludes evidence before the student
-- accepted the active classroom membership. First scope: TYT Mathematics v1.
BEGIN;
...
BEGIN
  ...
  v_integrity := public.curriculum_graph_integrity();
  IF v_integrity IS NULL
    OR jsonb_typeof(v_integrity) <> 'object'
    OR NOT (v_integrity ?& ARRAY[
      'total', 'mapped', 'unmapped', 'scopeMismatch', 'nodeOrphan', 'outcomeOrphan'
    ])
    ...
    OR (v_integrity->>'outcomeOrphan')::integer <> 0 THEN
...
REVOKE ALL ON FUNCTION public.get_institution_student_learning_analysis(
  uuid, uuid, text, text, text, timestamptz
) FROM PUBLIC, anon, authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_institution_student_learning_analysis(
  uuid, uuid, text, text, text, timestamptz
) TO service_role;
