...
CREATE TABLE IF NOT EXISTS public.paper_study_packs(
 id uuid PRIMARY KEY DEFAULT gen_random_uuid(),user_id uuid NOT NULL REFERENCES public.profiles(id) ON DELETE CASCADE,plan_id uuid NOT NULL REFERENCES public.daily_plan(id) ON DELETE RESTRICT,issue_request_id uuid NOT NULL UNIQUE,status text NOT NULL DEFAULT 'active' CHECK(status IN('active','submitted')),created_at timestamptz NOT NULL DEFAULT clock_timestamp(),expires_at timestamptz NOT NULL,submitted_at timestamptz,submit_result jsonb,UNIQUE(user_id,plan_id));
