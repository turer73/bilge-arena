    ...
    <div className="min-h-[100dvh] bg-[var(--app-bg)] pb-8 text-[var(--app-text)] md:bg-transparent md:pb-10">
      ...
      <header className="sticky top-0 z-30 border-b-2 border-[var(--app-border-soft)] bg-[var(--app-card)]/95 backdrop-blur-xl md:static md:border-0 md:bg-transparent md:backdrop-blur-none">
        <div className="mx-auto flex h-14 max-w-6xl items-center justify-between px-4 md:h-auto md:px-6 md:pb-5 md:pt-8 xl:px-8">
          ...
          <Link href="/arena/profil" aria-label="Çalışma ayarları" className="flex h-11 w-11 items-center justify-center rounded-2xl text-[var(--app-text-muted)] active:bg-[var(--app-hover)]">
            <Settings2 size={21} strokeWidth={2.5} />
          </Link>
        </div>
      </header>

      <div className="mx-auto w-full max-w-6xl px-3 pt-3 md:px-6 md:pt-0 xl:px-8">
        <section className="relative min-h-[154px] overflow-hidden rounded-[24px] bg-gradient-to-br from-[var(--app-accent)] to-[var(--app-accent-strong)] p-4 text-white shadow-[0_6px_0_var(--app-shadow-accent)] md:min-h-[170px] md:p-6">
          <div className="pointer-events-none absolute -right-10 -top-16 h-40 w-40 rounded-full border-[26px] border-white/10" />
          <div className="relative z-10 max-w-[68%] md:max-w-[72%]">
            ...
            <h2 className="mt-2 text-xl font-black leading-6 md:text-2xl">Bugün neyi ilerletelim?</h2>
            <p className="mt-1.5 text-xs font-semibold leading-5 text-white/80">Planın hazır; dersini seç ve kaldığın yerden devam et.</p>
          ...
          <div className="absolute -bottom-8 right-1 h-[150px] w-[118px] overflow-hidden md:right-5 md:w-[130px]">
        ...
        <div className="mt-4 grid items-start gap-4 lg:grid-cols-[minmax(0,1fr)_minmax(320px,380px)] lg:gap-6">
          <section aria-label="Çalışma planın" className="min-w-0 space-y-4">
            ...
            <Link href={`/arena/${game}`} className="flex min-h-14 items-center gap-3 rounded-[20px] border-2 border-[var(--app-success-border)] bg-[var(--app-card)] px-4 shadow-[0_4px_0_var(--app-success-border)] active:translate-y-0.5">
              ...
              <span className="min-w-0 flex-1"><span className="block text-xs font-black">Serbest pratik</span><span className="block text-[10px] font-semibold text-[var(--app-text-sub)]">Kendi modunu ve konunu seç</span></span>
