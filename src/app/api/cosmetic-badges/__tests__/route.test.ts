...
const request = () => new NextRequest('http://localhost:3141/api/cosmetic-badges')

describe('GET /api/cosmetic-badges', () => {
  beforeEach(() => {
    vi.clearAllMocks()
  ...
  it('yerelde yapılandırma yoksa rozet kataloğunu kullanılamaz olarak bildirir', async () => {
    const response = await GET(request())
    expect(response.status).toBe(200)
    expect(await response.json()).toEqual({ badges: [], unavailable: true })
