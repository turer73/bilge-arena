...
// Demo sorulari — Supabase baglanana kadar fallback
...
export function QuizEngine({ game }: QuizEngineProps) {
  ...
  const handleDenemeTimeUp = useCallback(() => {
    // Cevaplanmamis sorulari bos birak, sinavi bitir
  ...
  useEffect(() => {
    ...
    }, 1200) // 1.2sn dogru/yanlis goster, sonra ilerle
  ...
  const handleStart = useCallback(() => {
    // Oyuna gore filtrele, yoksa tum demo sorulari kullan
    let questions = DEMO_QUESTIONS.filter(q => q.game === game)
    ...
    if (isDeneme && denemeConfig) {
      ...
      // Son karistirma (sorular karisik sirali olsun)
      distributed.sort(() => Math.random() - 0.5)
      quizStore.startQuiz(distributed.slice(0, mode.questionCount))
      elapsed.reset()
    } else {
      // Normal mod
      const sliced = questions.slice(0, mode.questionCount)
      quizStore.startQuiz(sliced)
    }
  ...
  }, [game, mode, quizStore, timer, isDeneme, denemeConfig, elapsed])
  ...
  const handleAnswer = useCallback((optionIndex: number) => {
    ...
    if (isDeneme) {
      // Deneme: timer durmuyor, XP hesapla ama sure bazli bonus yok
  ...
  // Sonraki soru (normal mod icin — deneme otomatik ilerler)
    ...
    <div className="mx-auto max-w-[940px] p-4 lg:p-5">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="flex gap-3 animate-fadeUp" style={{ animationDelay: '0.08s', animationFillMode: 'both' }}>
          ...
          {/* Per-question timer kutusu (sadece normal modlar) */}
        ...
        {/* Aciklama paneli (sadece normal mod — deneme otomatik ilerler) */}
        {!isDeneme && quizStore.state === 'answered' && lastAnswer && (
          <>
            ...
            {/* Yorum bölümü */}
            {showComments && (
              <CommentSection questionId={question.id} isLoggedIn={false} />
            )}
      ...
      {/* Sag sidebar — sadece desktop, sadece normal mod */}
      {!isDeneme && (
