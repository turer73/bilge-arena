  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-[var(--surface)] p-4 sm:p-6">
      ...
      <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
        <Metric label="Aktif öğrenci" value={overview.summary.activeStudentCount} icon={<Users />} />
        <Metric label="Karar güvenli kanıt" value={overview.summary.studentsWithDecisionSafeEvidence} icon={<ShieldCheck />} />
        <Metric label="Destek ihtiyacı" value={overview.summary.studentsNeedingSupport} icon={<AlertTriangle />} tone="amber" />
        <Metric label="Kanıt yetersiz" value={overview.summary.studentsWithoutDecisionSafeEvidence} icon={<BarChart3 />} tone="muted" />
      </div>
          ...
          {overview.priorityOutcomes.length === 0 ? (
            ...
            <ul className="mt-3 space-y-2">
              {overview.priorityOutcomes.map((outcome) => (
                <li key={outcome.code} className="rounded-lg border border-white/10 p-3">
        ...
        <div className="rounded-xl border border-white/10 bg-white/[0.025] p-3 sm:p-4">
          ...
          <div className="mt-3 grid gap-2 sm:grid-cols-2">
            {Object.entries(overview.teacherIndicators.dimensions).map(([key, indicator]) => {
              ...
              return (
                <article key={key} className="rounded-lg border border-white/10 p-3">
...
function Metric({ label, value, icon, tone = 'primary' }: {
  label: string; value: number; icon: React.ReactElement; tone?: 'primary' | 'amber' | 'muted'
}) {
  const tones = { primary: 'text-[var(--primary)]', amber: 'text-amber-300', muted: 'text-[var(--text-sub)]' }
  return <div className="min-w-0 rounded-xl border border-white/10 bg-white/[0.025] p-3"><span className={`block [&>svg]:h-4 [&>svg]:w-4 ${tones[tone]}`}>{icon}</span><strong className="mt-2 block text-xl font-black">{value}</strong><span className="mt-0.5 block truncate text-[11px] text-[var(--text-sub)] sm:text-xs">{label}</span></div>
}
