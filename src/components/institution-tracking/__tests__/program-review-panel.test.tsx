...
beforeEach(()=>{
  vi.clearAllMocks()
  mocks.history.mockResolvedValue({programs:[{programRef:PROGRAM_REF,status:'published',weekStart:'2026-08-03',itemCount:4,publishedAt:'2026-08-02T09:00:00.000Z',reviewEligible:true,review:null}]})
...
describe('ProgramReviewPanel',()=>{
  it.each([320,375,390])('previews evidence and stores the teacher decision at %ipx',async(width)=>{
    ...
    const {container}=render(<ProgramReviewPanel classroomId={CLASSROOM_ID} memberRef={MEMBER_REF}/>)
    await user.click(await screen.findByRole('button',{name:'Kanıtı incele'}))
    expect(await screen.findByText(/Sistem önerisi:/)).toBeInTheDocument()
    ...
    expect(await screen.findByText('Kısmen etkili')).toBeInTheDocument()
