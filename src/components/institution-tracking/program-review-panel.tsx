...
const teacherResultCopy = {
  effective: 'Olumlu değişim gözlendi', partial: 'Kısmi olumlu değişim gözlendi', ineffective: 'Olumlu değişim gözlenmedi', insufficient: 'Kanıt yetersiz',
} as const
const suggestionCopy = {
  effective: 'Önceki sistem etiketi: Etkili', partial: 'Önceki sistem etiketi: Kısmen etkili', ineffective: 'Önceki sistem etiketi: Etkili değil', insufficient: 'Kanıt yetersiz',
  observed_improvement: 'Program haftası ve izleyen dönemde olumlu değişim gözlendi', mixed_observation: 'Program haftası ve izleyen dönemde karışık değişim gözlendi', no_observed_improvement: 'Program haftası ve izleyen dönemde olumlu değişim gözlenmedi',
} as const
type ResultCode = keyof typeof teacherResultCopy

export function ProgramReviewPanel({ classroomId, memberRef, game, examRef }: { classroomId: string; memberRef: string; game: GameSlug; examRef: string }) {
  ...
  useEffect(() => {
    ...
    queueMicrotask(async () => {
      ...
      try { setHistory(await fetchInstitutionStudentProgramHistory(classroomId, memberRef, game, examRef, controller.signal)) }
      catch (caught) {
        if (!(caught instanceof DOMException && caught.name === 'AbortError')) setError('Program geçmişi alınamadı.')
      }
    })
    return () => controller.abort()
  }, [classroomId, memberRef, game, examRef])

  async function openPreview(programRef: string) {
    setBusy(true); setError(null)
    try {
      const next = await previewInstitutionStudyProgramReview(programRef)
      setPreviewRef(programRef); setEvidence(next); setTeacherResult('insufficient'); setNote('')
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-[var(--surface)] p-4 sm:p-6">
      ...
      <p className="mt-1 text-xs leading-5 text-[var(--text-sub)]">Yalnız doğrulanmış oturumla tamamlanan görevlerin hedeflerindeki önce/sonra gözlemi gösterilir; bu, programın neden olduğu başarı kanıtı değildir. Nihai değerlendirmeyi öğretmen kaydeder.</p>
