...
import {institutionStudentReportScopeLabel,type InstitutionStudentReportMutation,type InstitutionStudentReportSnapshot} from '@/lib/institution-tracking/student-report'
...
export function StudentReportPanel({classroomId,memberRef,game,examRef}:{classroomId:string;memberRef:string;game:GameSlug;examRef:string}){
  ...
  useEffect(()=>{const controller=new AbortController();queueMicrotask(async()=>{if(controller.signal.aborted)return;setReports([]);setActive(null);setError(null);try{const next=await fetchInstitutionStudentReports(classroomId,memberRef,{game,displayExamRef:examRef},controller.signal);if(!controller.signal.aborted)setReports(next.reports)}catch(caught){if(!(caught instanceof DOMException&&caught.name==='AbortError'))setError('Rapor geçmişi alınamadı.')}});return()=>controller.abort()},[classroomId,memberRef,game,examRef])
  ...
  async function createReport(){setBusy(true);setError(null);try{const report=await createInstitutionStudentReport(classroomId,memberRef,{game,displayExamRef:examRef});setReports((current)=>[{reportRef:report.reportRef,snapshot:report.snapshot,createdAt:report.createdAt},...current].slice(0,10));setActive(report.snapshot)}catch{setError('Öğrenci durum raporu oluşturulamadı.')}finally{setBusy(false)}}
