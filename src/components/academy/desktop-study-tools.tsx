...
export function DesktopStudyTools({ game, examRef }: { game: GameSlug; examRef: string | null }) {
    ...
    {loading ? <p role="status">Çalışma araçların yükleniyor…</p> : !user
      ? <div className={styles.studyToolsGuest}>
          <div className={styles.studyToolPreviewGrid}>
            ...
            {examRef ? <button type="button" className={styles.studyToolPreview} onClick={() => setGuestDiagnosticOpen(true)}>
          ...
          {guestDiagnosticOpen && examRef
            ? <DiagnosticExplainerDialog game={game} examRef={examRef} onClose={() => setGuestDiagnosticOpen(false)} />
            : null}
        </div>
      : <div className={styles.studyToolsGrid}>
        <div className={styles.mainColumn}>
          <MasteryActionCard game={game} userId={user.id} examRef={examRef} diagnosticPresentation="explained" />
